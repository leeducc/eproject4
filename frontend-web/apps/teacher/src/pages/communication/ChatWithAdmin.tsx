...
const ChatWithAdmin: React.FC = () => {
  const [admins, setAdmins] = useState<UserStatus[]>([]);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        console.log("[ChatWithAdmin] Profile fetched:", profile);
        setCurrentUserId(profile.userId);
        
        fetchAdminList();
        connectWebSocket(profile.userId);
      } catch (err) {
        console.error('[ChatWithAdmin] Initialization failed', err);
      }
    };
    init();
  ...
  const fetchAdminList = async () => {
    try {
      ...
      console.log("[ChatWithAdmin] Admin staff list:", response.data);
  ...
  useEffect(() => {
    if (selectedAdminId) {
      fetchConversation(selectedAdminId);
      markAsRead(selectedAdminId);
    }
  }, [selectedAdminId, markAsRead]);
  ...
  const connectWebSocket = (userId: number) => {
    ...
    client.debug = (str) => {
        if (str.includes("CONNECTED") || str.includes("ERROR")) {
            console.log("[ChatWithAdmin] STOMP Debug:", str);
        }
    };
    
    const token = localStorage.getItem('teacher_token');
    client.connect({ 'Authorization': 'Bearer ' + token }, () => {
      console.log("[ChatWithAdmin] STOMP connected to user topic:", userId);
      
      
      client.subscribe(`/topic/chat/${userId}`, (message) => {
        const newMessage = JSON.parse(message.body);
        console.log("[ChatWithAdmin] New message received:", newMessage);
      ...
      client.subscribe('/topic/user-status', (statusMessage) => {
        ...
        console.log("[ChatWithAdmin] User status update received:", data);
        setAdmins(prevAdmins => prevAdmins.map(admin => 
          admin.id === data.userId ? { ...admin, isOnline: data.isOnline } : admin
        ));
      });
    });
  ...
  const handleSend = async () => {
    if ((!inputValue.trim() && !selectedFile) || !selectedAdminId) return;

    setIsUploading(true);
    const formData = new FormData();
    formData.append('receiverId', selectedAdminId.toString());
  ...
  const togglePin = async (adminId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await apiClient.post(`/chat/${adminId}/pin`);
      fetchAdminList();
    } catch (err) {
        ...
        <div className="w-80 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-gray-100 dark:border-slate-800 flex flex-col overflow-hidden">
          <div className="p-6 border-b border-gray-100 dark:border-slate-800">
            <h2 className="text-xl font-black mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
              <MessageSquare className="text-primary" /> Admins
            </h2>
              ...
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h4 className="font-bold text-sm truncate dark:text-white">{admin.fullName}</h4>
                    <div className="flex items-center gap-2">
                      ...
                      {admin.isPinned && <Pin size={12} className="text-primary fill-primary" />}
                    </div>
                  </div>
                  <p className="text-[10px] uppercase tracking-wider font-bold opacity-60">Admin Staff</p>
                </div>
                
                <div 
                  onClick={(e) => togglePin(admin.id, e)}
                  className={`p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all hover:bg-white dark:hover:bg-slate-700 shadow-sm ${admin.isPinned ? 'opacity-100' : ''}`}
                >
                  <Pin size={14} className={admin.isPinned ? 'text-primary fill-primary' : 'text-gray-400'} />
                </div>
              </button>
            ))}
          </div>
          ...
          {selectedAdminId ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-lg leading-tight dark:text-white">
                      {admins.find(a => a.id === selectedAdminId)?.fullName}
                    </h3>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <div className={`w-2 h-2 rounded-full ${admins.find(a => a.id === selectedAdminId)?.isOnline ? 'bg-green-500' : 'bg-gray-300'}`} />
                      <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        {admins.find(a => a.id === selectedAdminId)?.isOnline ? 'Active Now' : 'Offline'}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-6 bg-gray-50/10 dark:bg-slate-900">
              ...
              <div className="text-center max-w-xs">
                ...
                <p className="text-sm font-bold opacity-60 uppercase tracking-widest">Choose an admin to start</p>
              </div>
            </div>
          )}
        </div>
