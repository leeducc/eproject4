    ...
    const getStatusBadge = (status: EssayStatus, lockedBy?: string, lockedById?: string) => {
        switch (status) {
            ...
            case EssayStatus.IN_PROGRESS:
                ...
                return (
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${isMine ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-300">
            <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-bold text-gray-800 dark:text-slate-100">Essay Grading Queue</h2>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Manage and grade student submissions</p>
                </div>
                <div className="flex gap-4 items-center text-sm text-gray-600 dark:text-slate-400">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50 dark:bg-slate-800/50 text-gray-500 dark:text-slate-400 text-xs uppercase tracking-wider font-semibold">
                    ...
                    <tbody className="divide-y divide-gray-50 dark:divide-slate-800 text-sm">
                        {essays.map((essay) => {
                            ...
                            return (
                                <tr 
                                    key={essay.id} 
                                    className={`hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition-colors ${isLockedByOthers ? 'opacity-60 bg-gray-50/30 dark:bg-slate-800/20' : ''}`}
                                >
                                    <td className="px-6 py-5 font-medium text-gray-900 dark:text-slate-200">{essay.studentName}</td>
                                    <td className="px-6 py-5">
                                        <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${
                                            essay.taskType === 'TASK_1' ? 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300' : 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400'
                                        }`}>
